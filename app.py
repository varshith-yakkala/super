...
class Fact(BaseModel):
    ...
    evidence: str = Field(description="Short exact quote from the text, max 12 words")
...
class FactRelationship(BaseModel):
    ...
    relationship: Literal["CORROBORATES", "CONTRADICTS", "EXPLAINED_CONTRADICTION"] = Field(
        description="CORROBORATES: same truth. CONTRADICTS: mutually exclusive. EXPLAINED_CONTRADICTION: context makes them compatible."
    )
    reasoning: str = Field(description="One sentence explanation, max 15 words")
...
def calculate_fact_density(text: str) -> int:
    ...
    keywords   = len(re.findall(
        r'(?i)\b(revenue|profit|loss|margin|ebitda|growth|ceo|director|'
        r'increase|decrease|market.share|sales|capital|debt|equity|asset|'
        r'liability|percent|gdp|inflation|rate|forecast|quarter|annual)\b', text))
    return numbers + (currencies * 2) + (keywords * 3)

def _call_with_retry(payload_fn):
    last_err = None
...
def extract_facts_from_text(text: str, doc_name: str, page_num: int) -> List[Fact]:
    ...
    prompt = (
        "Extract the 2 most important numerical or semantic facts from the text below.\n"
        "Rules:\n"
        "  - Output EXACTLY 2 facts (or fewer if page has fewer meaningful facts).\n"
        "  - value: max 8 words.\n"
        "  - context: max 6 words.\n"
        "  - evidence: max 12 words, must be a substring of the text.\n"
        "  - id: unique short string like f1, f2.\n\n"
...
def compare_facts(new_facts: List[Fact], existing_facts: List[Fact]) -> List[FactRelationship]:
    ...
    # Cap existing facts to prevent context explosion
    ...
    prompt = (
        ...
        "Types:\n"
        "  CORROBORATES           - same entity+attribute, consistent values.\n"
        "  CONTRADICTS            - same entity+attribute, mutually exclusive values.\n"
        "  EXPLAINED_CONTRADICTION- appear to conflict but context explains difference.\n"
        "reasoning: one sentence, max 15 words.\n\n"
        f"NEW:\n{[slim(f) for f in new_facts]}\n\n"
        f"EXISTING:\n{[slim(f) for f in recent]}"
    )
    try:
        def call():
            return client.chat.completions.create(
...
with st.sidebar:
    ...
    if st.button("Process Documents", use_container_width=True):
        if not uploaded_files:
            ...
            with st.spinner("Analysing & extracting facts..."):
                for file in uploaded_files:
                    doc_name   = file.name
                    pages_text = extract_text_from_pdf(file)
                    if not pages_text:
                        st.warning(f"No readable text found in {doc_name}.")
                        continue
                    ...
                    chart_data = pd.DataFrame(
                        ...
                        index=[f"Page {i+1}" for i in range(len(pages_text))]
                    )
                    st.bar_chart(chart_data)

                    n_pages  = min(3, len(pages_text))
                    top_idxs = sorted(range(len(densities)), key=lambda i: densities[i], reverse=True)[:n_pages]
                    st.caption(f"Processing pages: {', '.join(str(i+1) for i in sorted(top_idxs))}")
...
with tab_facts:
    if st.session_state.facts:
        ...
        st.caption(f"Total facts: {len(st.session_state.facts)}")
    else:
...
with tab_rels:
    if st.session_state.relationships:
        ...
        def describe_fact(fid: str) -> str:
            f = id_to_fact.get(fid)
            if not f:
                return f"[Unknown ID: {fid}]"
            ctx = f" ({f.context})" if f.context else ""
            return f"{f.entity} - {f.attribute}: {f.value}{ctx}\n[Evidence: \"{f.evidence}\"]"
        ...
        st.caption(f"Total relationships: {len(st.session_state.relationships)}")
    else:
